...
class ProductsController < ApplicationController
  ...
  before_action :init_product_form_model, only: %i[new change_product_form]
  ...
  def show
    ...
    session[:product_recent_ids] = ([params[:id]] + (session[:product_recent_ids] || [])).uniq
    session[:product_recent_ids].pop if session[:product_recent_ids].size > 5
  end

  # GET /products/new
  def new
    @products = Product.all
    init_product_form_model
    @categories = Category.all
    ...
    session.delete(:num_form_variation)
  end
  ...
  def edit
    ...
    unless variations.empty?
      # params to pass to form
      @product_type = 'variation'
      @active = 4
      @expand = false
      @num_form_variation = variations.size

      # Variations
      @product_attrs = ProductAttributesName.all
      merge = helpers.products_cartesian(@product_attrs)
      @attrs_list_values = merge[:value]
      @attrs_list_values = @attrs_list_values.map do |attrs_values|
        attrs_values.map do |value|
          ProductAttributesValue.where(value: value).first
        end
      end

      @num_form_variation.times do
        variation = @product.product_variations.blank? ? @product.product_variations.build : @product.product_variations[0]
        variation_meta = variation.product_meta.nil? ? variation.build_product_meta : variation.product_meta
        variation_meta.build_product_detail if variation_meta.product_detail.nil?
        variation_meta.build_product_inventory if variation_meta.product_inventory.nil?
        variation_meta.build_product_shipping if variation_meta.product_shipping.nil?
        variation_meta.build_product_sale_price if variation_meta.product_sale_price.nil?
  ...
  def create
    ...
    if !sale_price.nil? && !sale_price.sale_date_start.nil? && !sale_price.sale_date_start.nil?
      ...
      time_end = Time.zone.parse(sale_price.sale_date_end_time || '00:00')
    ...
    respond_to do |format|
      if @product.save
        ...
        # Init form model to enable re submit
        init_product_form_model
        format.html { render :new, notice: 'Error has been occured.' }
        format.json { render json: @product.errors, status: :unprocessable_entity }
        # Setup variation if error form is variation product
        if session[:product_type] == 'variation'
          # params to pass to form
          @product_type = session[:product_type]
          @active = 4
          @expand = true
          @num_form_variation = session[:num_form_variation]
  ...
  def create_variations_from_attrs
    ...
    session[:num_form_variation] = 0
    @num_form_variation = params['num_form']
    session[:num_form_variation] =
      @num_form_variation + (session[:num_form_variation].nil? ? 0 : session[:num_form_variation])
        ...
        format.js do
          render 'products/response_update_form_add_to_cart',
                 locals: {
                   ...
                   image_url_update: variation.product_meta.images.first.url,
                   index: index,
  ...
  def init_product_form_model
    @product ||= Product.new(session[:product])
    @product_meta = if @product.product_meta.nil?
                      @product.build_product_meta
                    else
                      @product.product_meta
                    end
    @product_meta.build_product_detail if @product_meta.product_detail.nil?
    @product_meta.build_product_advanced if @product_meta.product_advanced.nil?
    @product_meta.build_product_inventory if @product_meta.product_inventory.nil?
    @product_meta.build_product_linked.product_upsells.build if @product_meta.product_linked.nil?
    @product_meta.build_product_extra if @product_meta.product_extra.nil?
    @product_meta.build_product_shipping if @product_meta.product_shipping.nil?
    @product_meta.build_product_sale_price if @product_meta.product_sale_price.nil?
    @categories = Category.all
    @product_attr = ProductAttributesName.new
  end
