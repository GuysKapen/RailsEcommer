...
(function ($, window, document, undefined) {
    ...
    AmsifySuggestags.prototype = {
        ...
        createHTML: function () {
            let HTML = '<div class="' + this.classes.sTagsArea.substring(1) + '"></div>';
            this.selectors.sTagsArea = $(HTML).insertAfter(this.selector);
            let labelHTML = '<div class="' + this.classes.inputArea.substring(1) + '"></div>';
            ...
            let sTagsInput = '<input type="text" class="' + this.classes.sTagsInput.substring(1) + '" placeholder="' + this.defaultLabel + '">';
            ...
            let listArea = '<div class="' + this.classes.listArea.substring(1) + '"></div>';
            ...
            let list = '<ul class="' + this.classes.list.substring(1) + '"></ul>';
        ...
        setTagEvents: function () {
            ...
            $(this.selectors.sTagsInput).keyup(function (e) {
                ...
                let isDelimiter = ($.inArray(key, _self.settings.delimiters) !== -1);
                if (key === 'Enter' || key === ',' || isDelimiter) {
                    let value = $.trim($(this).val().replace(/,/g, ''));
                    if (isDelimiter) {
                    ...
                    let removeClass = _self.classes.readyToRemove.substring(1);
        ...
        getValue: function (tag) {
            if (this.settings.suggestions.length) {
                let value = tag;
                let lower = tag.toString().toLowerCase();
                $.each(this.settings.suggestions, function (key, item) {
        ...
        processAjaxSuggestion: function (value, keycode) {
            ...
            let params = {existingTags: this.tagNames, existing: this.settings.suggestions, term: value};
            let ajaxFormParams = {
                url: actionURL,
            };
            ...
            if (keycode === '40' || keycode === '38') {
                ...
                this.delayTimer = setTimeout(function () {
                    if (_self.isSuggestAction() && !_self.ajaxActive) {
                        let minChars = _self.settings.suggestionsAction.minChars;
                        let minChange = _self.settings.suggestionsAction.minChange;
                        let lastSearch = _self.selectors.sTagsInput.attr('last-search');
        ...
        upDownSuggestion: function (value, type) {
            ...
            if (!isActive) {
                let childItem = (type === 'down') ? 'first' : 'last';
        ...
        suggestWhiteList: function (value, keycode, showAll) {
            ...
            let $list = $(this.selectors.listArea).find(this.classes.list);
        ...
        setDefault: function () {
            let _self = this;
            let items = $(this.selector).val().split(',');
            if (items.length) {
        ...
        addTag: function (value, animate = true) {
            ...
            let html = '<span class="' + this.classes.tagItem.substring(1) + '" data-val="' + value + '">' + this.getTag(value) + ' ' + this.setIcon() + '</span>';
            ...
            let itemKey = this.getItemKey(value);
        ...
        flashItem: function (value) {
            ...
            $(this.selectors.sTagsArea).find(this.classes.tagItem).each(function () {
                let tagName = $.trim($(this).attr('data-val'));
        ...
        setIcon: function () {
            let removeClass = this.classes.removeTag.substring(1);
        ...
        setInputValue: function () {
            this.updateIsRequired();
            // const ids = this.settings.suggestions.indexOf(value)
            const values = this.tagNames.map(it => this.settings.valueMaps[this.settings.suggestions.indexOf(it)])

            console.log("Add", values)
            // $(this.selector).val(this.tagNames.join(','));
            $(this.selector).val(values.join(','));
        ...
        checkMethod: function () {
            ...
            $(this.selector).show();
            return !(typeof this.method !== undefined && this.method === 'destroy');
        },
        ...
        isSimilarText: function (str1, str2, perc) {
            ...
            let percent = this.similarity(str1, str2);
        ...
        checkPlusAfter: function (fromBlur) {
            if (this.settings.showPlusAfter > 0) {
                if (this.tagNames.length > this.settings.showPlusAfter) {
                    let _self = this;
                    let plusNumber = this.tagNames.length - this.settings.showPlusAfter;
                    if (!this.selectors.plusTag) {
                        let html = '<span class="' + this.classes.plusItem.substring(1) + '"></span>';
    ...
    $.fn.amsifySuggestags = function (options, method) {
        return this.each(function () {
            let amsifySuggestags = new AmsifySuggestags(this);
